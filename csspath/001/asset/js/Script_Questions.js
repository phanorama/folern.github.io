...
const restart_quiz = result_box.querySelector(".buttons .restart_result");
const quit_quiz = result_box.querySelector(".buttons .quit_result");
...
function showResult(){
    ...
    let scoreTag = '<span>Nilai Quiz (<p>'+ userScore+'</p>/<p>'+questions.length +'</p>)</span>';
        ...
        if(time < 0){
            ...
            for(i=0; i < allOptions; i++){
                if(option_list.children[i].textContent == correcAns){
                    option_list.children[i].setAttribute("class", "option correct");
                    option_list.children[i].insertAdjacentHTML("beforeend", tickIconTag);
                    console.log("Waktu Habis: memilih jawaban yang benar secara otomatis.");
                }
            }
            for(i=0; i < allOptions; i++){
                option_list.children[i].classList.add("disabled");
